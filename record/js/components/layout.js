...
// "calendar" 항목도 "home"과 같은 방식으로 슬롯을 공유함: 파견 기간 중(오늘이
// 파견 시작일~종료일 사이)에는 준비 할일 캘린더 대신 사진 기록(다이어리)으로 라벨·href가
// 통째로 바뀜. 출국 전 = 할일 체크, 출국 후 = 사진 기록. 항목 수는 그대로 4개.
...
function renderAppNav(activeKey) {
  ...
  const inProgram = typeof AppState.isDateInProgram === 'function' && AppState.isDateInProgram(todayIso);
  const calendarHref = inProgram ? 'diary.html' : 'calendar.html';
  const calendarLabel = inProgram ? '기록' : '캘린더';
  ...
  mount.innerHTML = `
    ...
    <nav class="app-nav__links">
      ${LAYOUT_NAV_ITEMS.map(item => {
        ...
        const isCalendarSlot = item.key === 'calendar';
        const href = isHomeSlot ? homeHref : isCalendarSlot ? calendarHref : item.href;
        const label = isHomeSlot ? homeLabel : isCalendarSlot ? calendarLabel : item.label;
        const isActive = activeKey === item.key
          || (isHomeSlot && activeKey === 'prepare')
          || (isCalendarSlot && activeKey === 'diary');
